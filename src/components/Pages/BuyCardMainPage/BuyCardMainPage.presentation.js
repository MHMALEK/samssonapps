          ...
          <div className="first-form-wrapper">
            ...
            return  (
              <Card key={card.id} title={card.title}>
                ...
                {
                  card.educationSystem && card.educationSystem.length > 0 && 
                          <div  onClick={() => this.openModal(card.id, 'EducationSystem') }>
                              <p className="select-without-option-lable">نظام آموزشی</p>
                              <SelectWithOutOption />
                              {this.state[card.id+'modalIsOpenEducationSystem'] && 
                              <ModalComponent>
                              {card.educationSystem.map((educationSystem, index) => {
                                return <p key={index}>{educationSystem.name}</p>
                              })}
                            </ModalComponent>}
                ...
                {
                  card.teachingInstitution && card.teachingInstitution.length > 0 && 
                          <div  onClick={() => this.openModal(card.id, 'TeachingInstitution')}>
                              <p className="select-without-option-lable"> مجموعه آزمایشی</p>
                              <SelectWithOutOption />
                              {this.state[card.id+'modalIsOpenTeachingInstitution'] && 
                               <ModalComponent >
                               {card.teachingInstitution.map((teachingInstitution, index) => {
                                 return <p key={index}>{teachingInstitution.name}</p>
                               })}
                             </ModalComponent>
                            }
